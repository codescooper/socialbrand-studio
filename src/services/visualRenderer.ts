...
export const renderVisualBlob = async (image: HTMLImageElement, brandKit: BrandKit, type: ExportType, quality = 0.9, onLogoError?: () => void): Promise<Blob> => {
  ...
  canvas.width = t.outputWidth;
  canvas.height = t.outputHeight;
  const ctx = canvas.getContext("2d");
  if (!ctx) throw new Error("canvas-context");
  ctx.fillStyle = brandKit.secondaryColor;
  ...
  ctx.fillStyle = brandKit.textColor;
  ctx.textBaseline = "middle";
  ctx.textAlign = "left";
  ctx.font = `700 ${(canvas.width * t.nameSize) / 100}px Arial`;
  ctx.fillText(brandKit.brandName, (canvas.width * t.nameX) / 100, (canvas.height * t.nameY) / 100);
  if (t.showContact) {
    ctx.textAlign = "right";
    ctx.font = `500 ${(canvas.width * t.contactSize) / 100}px Arial`;
    ctx.fillText(`${brandKit.contact} • ${brandKit.website}`, (canvas.width * t.contactX) / 100, (canvas.height * t.contactY) / 100);
  }
  if (brandKit.logo && t.showLogo) {
    try {
      const logo = await decodeDataImage(brandKit.logo);
      const width = (canvas.width * t.logoSize) / 100;
      const height = (logo.height / logo.width) * width;
      ctx.drawImage(logo, (canvas.width * t.logoX) / 100, (canvas.height * t.logoY) / 100 - height / 2, width, height);
    } catch {
      onLogoError?.();
    }
  }
