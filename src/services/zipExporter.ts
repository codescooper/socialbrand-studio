...
export const uniqueOutputNames = (files: BatchFile[], extension: "png" | "jpg") => {
  ...
  return files.map((file) => {
    const stem = safeFilename(file.name.replace(/\.[^.]+$/, "")); const count = (used.get(stem) || 0) + 1; used.set(stem, count);
    return `${stem}${count > 1 ? `-${count}` : ""}.${extension}`;
  });
};

export const buildBatchReport = (files: BatchFile[], settings: BatchSettings, date = new Date()) => JSON.stringify({
  generatedAt: date.toISOString(), brandKit: settings.brandKit.brandName,
...
export const batchZipFilename = (settings: BatchSettings, date = new Date()) => `socialbrand-${safeFilename(settings.brandKit.brandName)}-${safeFilename(SOCIAL_FORMATS[settings.socialFormatKey]?.label || settings.socialFormatKey)}-${date.toISOString().slice(0, 10)}.zip`;
