...
function repository<T extends { id: string }>(table: EntityTable<T, "id">) {
  return {
    list: () => table.toArray(), getById: (id: string) => table.get(id as never), count: () => table.count(),
    create: async (record: T) => { try { await table.add(record as never); return record; } catch (e) { throw storageError(e, "la création"); } },
    update: async (record: T) => { try { await table.put(record as never); return record; } catch (e) { throw storageError(e, "la sauvegarde"); } },
    delete: (id: string) => table.delete(id as never), clear: () => table.clear()
  };
}
...
export const settingsRepository = {
  list: () => db.settings.toArray(), getById: (key: string) => db.settings.get(key), count: () => db.settings.count(), clear: () => db.settings.clear(), delete: (key: string) => db.settings.delete(key),
  update: async (record: AppSettingRecord) => { try { await db.settings.put(record); return record; } catch (e) { throw storageError(e, "les réglages"); } }
};
