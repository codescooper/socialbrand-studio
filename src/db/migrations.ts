...
const record = (kit: BrandKit, createdAt = new Date().toISOString()): BrandKitRecord => ({ id: kit.id, version: 1, name: kit.brandName, content: kit, createdAt, updatedAt: createdAt });
    ...
    if (!done) {
      ...
      for (const key of LEGACY_KEYS) {
        ...
        if (key === "sbs-brand-kits") {
          try { const parsed = JSON.parse(raw); if (!Array.isArray(parsed)) throw new Error(); for (const item of parsed) { const result = validateBrandKitImport(item); if (!result) { report.ignored++; continue; } if (seen.has(result.brandKit.id) || await db.brandKits.get(result.brandKit.id)) continue; seen.add(result.brandKit.id); await db.brandKits.add(record(result.brandKit)); report.migrated++; if (result.repaired) report.repaired++; } } catch { report.ignored++; }
        } else { try { const result = validateBrandKitImport(JSON.parse(raw)); if (!result) { report.ignored++; continue; } const kit = result.brandKit; if (seen.has(kit.id) || await db.brandKits.get(kit.id)) continue; seen.add(kit.id); await db.brandKits.add(record(kit)); report.migrated++; if (result.repaired) report.repaired++; } catch { report.ignored++; } }
      }
