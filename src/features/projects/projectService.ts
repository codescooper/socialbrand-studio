...
export async function saveProject(input: { id?: string; name: string; brandKit: BrandKit; socialFormat: string; exportFormat: ExportType; jpgQuality: number; image?: File }) {
  ...
  try {
    await db.transaction("rw", db.projects, db.assets, async () => {
      if (input.image) {
        sourceAssetId = sourceAssetId || crypto.randomUUID();
        await db.assets.put({
          id: sourceAssetId,
          type: "source",
      ...
      const project: ProjectRecord = {
        ...
        renderSettings: input.brandKit.template,
        socialFormat: input.socialFormat,
        exportFormat: input.exportFormat,
        jpgQuality: input.jpgQuality,
        sourceAssetId,
        createdAt: existing?.createdAt || now,
        updatedAt: now,
        lastOpenedAt: now,
    ...
    throw storageError(e, "le projet");
  }
}
export async function duplicateProject(project: ProjectRecord) {
  const now = new Date().toISOString();
  const copy = { ...project, id: crypto.randomUUID(), name: `${project.name} — copie`, createdAt: now, updatedAt: now, lastOpenedAt: now };
