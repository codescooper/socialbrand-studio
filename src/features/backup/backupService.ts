...
const JSON_FILES: Record<Exclude<keyof BackupData, "assets">, string> = {
  brandKits: "data/brand-kits.json",
...
export async function exportBackup() {
  ...
  const assetMeta = data.assets.map(({ blob: _blob, ...asset }) => asset);
  zip.file("data/assets.json", JSON.stringify(assetMeta, null, 2));
...
export async function parseBackup(file: File): Promise<{ manifest: BackupManifest; data: BackupData }> {
  if (!file.name.toLowerCase().endsWith(".zip") || file.size > 500 * 1024 * 1024)
    throw new StorageError("Sauvegarde invalide ou trop volumineuse. Les données actuelles sont conservées.", "invalid-backup");
  try {
    const zip = await JSZip.loadAsync(await file.arrayBuffer());
    const manifest = JSON.parse(await required(zip, "manifest.json").async("text")) as BackupManifest;
    ...
    throw new StorageError("Archive corrompue ou incomplète. Aucune donnée n’a été modifiée.", "invalid-backup", { cause: error });
  }
}
function required(zip: JSZip, path: string) {
