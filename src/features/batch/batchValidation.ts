...
export const selectNewBatchFiles = (existing: BatchFile[], incoming: File[]) => {
  ...
  for (const file of incoming) {
    if (fingerprints.has(fileFingerprint(file))) { rejected.push({ file, reason: "Doublon ignoré (même nom, taille et date)." }); continue; }
    if (existingAccepted.length + accepted.length >= MAX_BATCH_FILES) { rejected.push({ file, reason: `Limite de ${MAX_BATCH_FILES} images atteinte.` }); continue; }
    const fileError = validateImageFile(file);
    if (fileError === "unsupported") { rejected.push({ file, reason: "Format non supporté. Utilisez PNG, JPG, WEBP ou AVIF." }); continue; }
    if (fileError === "too-large") { rejected.push({ file, reason: "Fichier supérieur à 25 Mo." }); continue; }
    if (totalSize + file.size > MAX_BATCH_TOTAL_SIZE) { rejected.push({ file, reason: "Ce fichier ferait dépasser la limite totale de 200 Mo." }); continue; }
    fingerprints.add(fileFingerprint(file)); totalSize += file.size; accepted.push(file);
  }
